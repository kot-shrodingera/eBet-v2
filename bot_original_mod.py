...
from bot.browser.ChromeController import ChromeRemoteDebugInterface, ChromeContext
...
with ChromeContext(binary=binary_path, additional_options=additional_options) as browser:
# browser = ChromeRemoteDebugInterface(binary=binary_path, additional_options=additional_options)
    ...
    # Get old bets
    response = requests.post('http://bvb.strike.ws/bot/index.php?api[method]=get_forks&api[version]=2', files=auth).json()
    # printr('Response:', response)
        ...
        for bet_details in response['data']['forks']:
            ...
            if bet_details not in placed_bets:
                ...
                if len(selection_details) == 2:
                    ...
                    market, alternative_selection_name, column, selection = selection_details
                ...
                result = browser.execute_javascript_function(f'''
                    function get_bet_button() {{
                        ...
                        if ('{column}' == 'Over') {{
		                    ...
		                    var bet_button = document.evaluate(`.//*[text()="${selection}"]`, market).iterateNext().nextElementSibling;
                            }}
